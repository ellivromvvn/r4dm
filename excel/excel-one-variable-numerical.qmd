...
t.test(
  ...
  tidy() |>
  select(estimate, statistic, method, alternative) |>
  kable()
```

The test statistics is shown in @tbl-stats. Based on the results and the *p*-value being lesser than 5% (*p*-value = 0.02), we reject the null hypothesis. There is sufficient evidence that the mean sepal length is greater than the expected average of 5.7 cm.
:::
